...
import DeepWaveTools.imot_tools.math.special as ispecial
...
class Tukey:
    ...
    .. testsetup::
       ...
       from DeepWaveTools.imot_tools.math.func import Tukey
...
class SphericalDirichlet:
    ...
    def _exact_kernel(self, x):
        amplitude = special.eval_legendre(self._N + 1, x) - special.eval_legendre(self._N, x)
...
class Kent:
    ...
    def __call__(self, x):
        ...
        pdf = (
            np.exp(
                (x @ self._g1) - 1 + 0.5 * self._beta * ((x @ self._g2) ** 2 - (x @ self._g3) ** 2)
            )
            ** self._k
        )
        pdf /= self._cst

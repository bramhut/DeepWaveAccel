...
# Fixed point version of the Goertzel algorithm
def goertzel_multi_bin_fxp(x, bins, Nf, fxp_sn, fxp_coef, fxp_output, overlap=0.5, hann=True, normalize=True, logger=None):
    """
    Compute the DFT using the Goertzel algorithm for multiple frequency bins,
    returning individual results per bin. Uses fixed-point arithmetic.
    ...
    Nf : int
        Frame length
    fxp_sn : Fxp.dtype
        Fixed-point configuration for s[n]
    fxp_coef : Fxp.dtype
        Fixed-point configuration for coefficients
    fxp_output : Fxp.dtype
        Fixed-point configuration for output
    overlap : float
        Overlap fraction [0, 1)
    hann : bool
        Apply Hann window
    ...
    # Check if input is fixed-point
    if not isinstance(x, Fxp):
        raise TypeError("Input signal x must be a fixed-point array (Fxp).")
    ...
    X_bins = np.zeros((n_frames, n_ch, n_bins), dtype=fxp_output)
    ...
    if hann:
        window = 0.5 * (1 - np.cos(2 * np.pi * np.arange(Nf) / (Nf - 1)))
        window = Fxp(window[:, np.newaxis], dtype=fxp_coef)
        window.info(verbose=3)

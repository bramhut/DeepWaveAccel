...
architecture behave of deepwaveaccel_CTRL_BUS_s_axi is
    ...
    constant ADDR_GOER_CFG_BASE : INTEGER := 16#200#;
    constant ADDR_GOER_CFG_HIGH : INTEGER := 16#3ff#;
    ...
    -- memory signals
    signal int_goer_cfg_address0 : UNSIGNED(5 downto 0);
    signal int_goer_cfg_ce0    : STD_LOGIC;
    signal int_goer_cfg_q0     : UNSIGNED(63 downto 0);
    signal int_goer_cfg_address1 : UNSIGNED(5 downto 0);
    signal int_goer_cfg_ce1    : STD_LOGIC;
    signal int_goer_cfg_be1    : UNSIGNED(7 downto 0);
    signal int_goer_cfg_we1    : STD_LOGIC;
    signal int_goer_cfg_d1     : UNSIGNED(63 downto 0);
    signal int_goer_cfg_q1     : UNSIGNED(63 downto 0);
    signal int_goer_cfg_read   : STD_LOGIC;
    signal int_goer_cfg_write  : STD_LOGIC;
    signal int_goer_cfg_shift1 : UNSIGNED(0 downto 0);

    component deepwaveaccel_CTRL_BUS_s_axi_ram is
        generic (
    ...
    end component deepwaveaccel_CTRL_BUS_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_goer_cfg
int_goer_cfg : deepwaveaccel_CTRL_BUS_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_goer_cfg_read = '0') else '0';
    RVALID    <= RVALID_t;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    rdata_data <= (others => '0');
                elsif (int_goer_cfg_read = '1') then
                    rdata_data <= RESIZE(SHIFT_RIGHT(int_goer_cfg_q1, TO_INTEGER(int_goer_cfg_shift1)*32), 32);
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- goer_cfg
    int_goer_cfg_address0 <= RESIZE(UNSIGNED(goer_cfg_address0), 6);
    int_goer_cfg_ce0     <= goer_cfg_ce0;
    goer_cfg_q0          <= STD_LOGIC_VECTOR(RESIZE(int_goer_cfg_q0, 64));
    int_goer_cfg_address1 <= raddr(8 downto 3) when ar_hs = '1' else waddr(8 downto 3);
    int_goer_cfg_ce1     <= '1' when ar_hs = '1' or (int_goer_cfg_write = '1' and WVALID  = '1') else '0';
    int_goer_cfg_we1     <= '1' when int_goer_cfg_write = '1' and w_hs = '1' else '0';
    int_goer_cfg_be1     <= SHIFT_LEFT(RESIZE(UNSIGNED(WSTRB), 8), TO_INTEGER(waddr(2 downto 2)) * 4) when int_goer_cfg_we1 = '1' else (others=>'0');
    int_goer_cfg_d1      <= RESIZE(UNSIGNED(WDATA) & UNSIGNED(WDATA), 64);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ar_hs = '1' and raddr >= ADDR_GOER_CFG_BASE and raddr <= ADDR_GOER_CFG_HIGH) then
                    int_goer_cfg_read <= '1';
                else
                    int_goer_cfg_read <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_goer_cfg_write <= '0';
            elsif (ACLK_EN = '1') then
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_GOER_CFG_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_GOER_CFG_HIGH) then
                    int_goer_cfg_write <= '1';
                elsif (w_hs = '1') then
                    int_goer_cfg_write <= '0';
                end if;
            end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_goer_cfg_shift1 <= (others=>'0');
            elsif (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    int_goer_cfg_shift1 <= raddr(2 downto 2);
                end if;
            end if;
        end if;
    end process;
...
entity deepwaveaccel_CTRL_BUS_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity deepwaveaccel_CTRL_BUS_s_axi_ram;

architecture behave of deepwaveaccel_CTRL_BUS_s_axi_ram is
